...
function Analytics() {
  ...
  const healthyPct = distributionTotal ? Math.round((Number(data?.distribution?.healthy ?? 0) / distributionTotal) * 100) : 0;
  const monitoringPct = distributionTotal ? Math.round((Number(data?.distribution?.monitoring ?? 0) / distributionTotal) * 100) : 0;
  const criticalPct = distributionTotal ? Math.max(0, 100 - healthyPct - monitoringPct) : 0;
