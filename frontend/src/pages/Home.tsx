...
function LocalAuthPanel({ navigate, language }: { navigate: (path: string) => void; language: LanguageCode }) {
  ...
  const detectSignupLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition((position) => {
      ...
      if (window.google?.maps) new google.maps.Geocoder().geocode({ location: { lat: latitude, lng: longitude } }, (results, status) => { if (status !== "OK" || !results?.[0]) return; const values = new Map<string, string>(); results[0].address_components.forEach((part) => part.types.forEach((type) => values.set(type, part.long_name))); setForm((current) => ({ ...current, region: results[0].formatted_address, state: values.get("administrative_area_level_1") ?? current.state, district: values.get("administrative_area_level_2") ?? values.get("administrative_area_level_3") ?? current.district, pinCode: values.get("postal_code") ?? current.pinCode, village: values.get("locality") ?? values.get("sublocality_level_1") ?? values.get("administrative_area_level_3") ?? current.village })); });
      setGpsState("saved");
      toast.success("GPS location captured and address fields filled. Save the account to keep it on your profile.");
    }, () => {
      setGpsState("denied");
