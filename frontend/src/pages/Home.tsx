...
type ScanResult = { scanId: number; cropType: string; disease: string; riskLevel: string; confidence: number; assessment: string; symptoms: string[]; recommendations: string[] };
...
function ScanFlow({ onComplete, canAnalyze }: { onComplete: (scanId: number) => void; canAnalyze: boolean }) {
  const [step, setStep] = useState<"start" | "preview" | "analyzing" | "result">("start");
  ...
  const resetScan = () => { setStep("start"); setResult(null); setFileName(""); setPreviewUrl(""); setImageBase64(""); setSelectedCropId(""); };
  if (step === "analyzing") return <div className="scan-state centered"><div className="spinner"><RefreshCw size={30} /></div><p className="eyebrow">AI SCAN ANALYSIS</p><h1>Understanding your crop</h1><p>We’re reviewing the image and preparing practical guidance for your field.</p><div className="progress-steps"><span className="done"><Check size={14} /> Uploading image</span><span className="active"><Activity size={14} /> Checking visible symptoms</span><span>Preparing next steps</span><span>Saving scan history</span></div></div>;
  if (step === "result" && result) { const nextSteps = getScanNextSteps(result.riskLevel, result.disease); const profile = snapshot.data?.profile; return <div className="page-stack scan-results"><div className="result-banner"><Check size={22} /><div><p className="eyebrow">SCAN COMPLETE</p><h1>Your assessment is ready</h1><p>It’s saved in Scan History. Save it to Cases when you want a follow-up record.</p></div></div><section className="surface-card result-card"><div className="result-top"><div><StatusChip tone={result.riskLevel === "high" || result.riskLevel === "critical" ? "high" : result.riskLevel === "medium" ? "medium" : "healthy"}>{result.riskLevel.toUpperCase()} RISK</StatusChip><h2>{result.cropType || "Crop"} assessment</h2><p className="diagnosis-line"><b>Diagnosis:</b> {result.disease || "No disease identified"}</p><p>Assessment generated from the uploaded image by the secure server-side analysis service.</p></div><div className="confidence"><strong>{result.confidence}%</strong><span>CONFIDENCE</span></div></div><div className="result-grid"><div><span className="eyebrow">WHAT WE NOTICED</span><p>{result.symptoms?.join(" ") || result.assessment}</p></div><div><span className="eyebrow">ASSESSMENT SUMMARY</span><p>{result.assessment || "Structured crop assessment available."}</p></div></div><section className="next-steps-card"><div className="next-steps-heading"><span className="next-steps-icon"><ArrowRight size={17} /></span><div><p className="eyebrow">WHAT TO DO NEXT</p><h3>Practical steps for this crop</h3></div></div><ol>{nextSteps.map((nextStep) => <li key={nextStep}>{nextStep}</li>)}</ol></section><div className="scan-result-actions"><Button onClick={() => onComplete(result.scanId)}><ClipboardList size={17} /> Save to Cases</Button><Button variant="outline" onClick={resetScan}><Camera size={16} /> Scan another crop</Button><Button variant="ghost" asChild><Link href="/farmer/cases">Open Cases <ChevronRight size={16} /></Link></Button></div></section><NearbyExperts state={profile?.state} district={profile?.district} /></div>; }
  return <div className="page-stack scan-page"><button className="back-link scan-back" type="button" onClick={() => toast.info("Use the workspace navigation to leave scan mode.")}>← Back to dashboard</button><section className="scan-intro"><div className="scan-orb"><Camera size={34} /></div><p className="eyebrow">CROP HEALTH SCAN</p><h1>Get a clear read on your crop</h1><p>Take a focused photo of a leaf, stem, or affected area. We’ll return an assessment, practical next steps, and support options.</p></section>{step === "start" ? <section className="surface-card upload-stage"><div className="upload-stage-heading"><div className="step-number">01</div><div><p className="eyebrow">START WITH A PHOTO</p><h2>Choose how to add your crop image</h2><p>Use natural light and keep the affected area in focus.</p></div></div><div className="upload-options"><label className="upload-card primary-upload"><input type="file" accept="image/jpeg,image/png,image/webp" capture="environment" onClick={(e) => { e.currentTarget.value = ""; }} onChange={(e) => void chooseFile(e.target.files?.[0])} /><span className="upload-card-icon"><Camera size={23} /></span><b>Take a photo</b><span>Open your device camera</span></label><label className="upload-card"><input type="file" accept="image/jpeg,image/png,image/webp" onClick={(e) => { e.currentTarget.value = ""; }} onChange={(e) => void chooseFile(e.target.files?.[0])} /><span className="upload-card-icon"><CloudUpload size={23} /></span><b>Upload from gallery</b><span>Choose an existing image</span></label></div><div className="upload-checklist"><span><Check size={15} /> One crop per photo</span><span><Check size={15} /> Leaf or stem fills the frame</span><span><Check size={15} /> JPEG, PNG, or WebP · max 8 MB</span></div></section> : <section className="surface-card preview-card"><div className="preview-card-heading"><div><p className="eyebrow">STEP 02 · REVIEW PHOTO</p><h2>Is this image ready?</h2><p>Make sure the affected crop area is clearly visible before analysis.</p></div><button className="icon-button" type="button" onClick={resetScan} aria-label="Replace image"><X size={18} /></button></div><div className="preview-placeholder">{previewUrl ? <img className="preview-image" src={previewUrl} alt="Selected crop" /> : <Leaf size={54} />}</div><div className="photo-ready"><div className="photo-ready-icon"><Check size={17} /></div><div><b>Photo ready to review</b><span>Check focus, lighting, and that the affected area is visible.</span></div></div><div className="preview-file"><span className="file-badge"><Leaf size={16} /></span><div><b>{fileName}</b><small>Resized securely for faster analysis</small></div></div>{cropOptions.data?.length ? <label className="scan-crop-select">Link to a crop record <span className="optional-label">optional</span><select value={selectedCropId} onChange={(event) => setSelectedCropId(event.target.value)}><option value="">Choose a crop</option>{cropOptions.data.map((crop) => <option value={crop.id} key={crop.id}>{crop.name} · {crop.cropType}</option>)}</select></label> : <div className="scan-note"><Leaf size={16} /><span>No crop records yet. You can still analyze this photo and save the result to Cases.</span></div>}<div className="button-row scan-actions"><Button variant="outline" onClick={resetScan}><X size={16} /> Replace photo</Button><Button disabled={analyze.isPending || !canAnalyze} onClick={async () => { if (!canAnalyze) { toast.error("Sign in to run a real crop analysis."); return; } setStep("analyzing"); try { const assessment = await analyze.mutateAsync({ imageBase64, mimeType, fileName, cropId: selectedCropId ? Number(selectedCropId) : undefined }); setResult(assessment); setStep("result"); } catch (error) { setStep("preview"); const message = error instanceof Error ? error.message : "The scan could not be completed. Please try again."; toast.error(message, { duration: 9000 }); } }}>{analyze.isPending ? "Analyzing…" : canAnalyze ? "Analyze image" : "Sign in to analyze"} <ArrowRight size={16} /></Button></div></section>}</div>;
}
...
function CaseList({ admin = false }: { admin?: boolean }) {
  ...
  return <div className="page-stack"><div className="admin-title"><p className="eyebrow">{admin ? "REVIEW QUEUE" : "CASE MANAGEMENT"}</p><h1>{admin ? "Case Review" : "My Cases"}</h1><p>{admin ? "Review approved case records and follow up on high-risk findings." : "Review your crop assessments and recommended next steps."}</p></div><div className="tab-row"><button className="active">All</button><button>High Risk</button><button>Resolved</button></div><section className="surface-card">{loading ? <div className="empty-state"><RefreshCw className="spin" size={22} /><p>Loading cases…</p></div> : error ? <EmptyState title="Cases unavailable" detail="We could not load case records. Please try again." action={<Button onClick={() => adminCases.refetch()}>Retry</Button>} /> : rows?.length ? <div className="activity-list">{rows.map((item) => <div key={item.id}><div className={`activity-icon ${item.status === "resolved" ? "mint" : "danger"}`}><ClipboardList size={17} /></div><p><b>{item.reference}</b><span>{item.status === "resolved" ? "Resolved" : "Open case requiring review"}</span><small>{new Date(item.createdAt).toLocaleString()}</small></p></div>)}</div> : <EmptyState title={admin ? "No approved cases" : "No cases yet"} detail={admin ? "Approved case records will appear here after review." : "Run a crop scan to create your first case."} action={!admin ? <Button asChild><Link href="/farmer/scan">Start a scan</Link></Button> : undefined} />}</section></div>;
}
...
function LocalAuthPanel({ navigate }: { navigate: (path: string) => void }) {
  ...
  const [form, setForm] = useState({ name: "", email: "", password: "", phone: "", region: "", state: "", district: "", pinCode: "", village: "", primaryCrop: "", farmingExperienceYears: "" });
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    try {
      const result = mode === "signin" ? await signin.mutateAsync({ email: form.email, password: form.password }) : await signup.mutateAsync({ ...form, role, farmingExperienceYears: form.farmingExperienceYears ? Number(form.farmingExperienceYears) : undefined });
