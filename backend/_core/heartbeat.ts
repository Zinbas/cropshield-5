...
const buildEndpoint = (rpc: string): string => {
  if (!(process.env.EXTERNAL_SERVICE_URL ?? "")) {
    throw new TRPCError({
      ...
      message: "Heartbeat service URL is not configured (EXTERNAL_SERVICE_URL).",
    });
  }
  if (!(process.env.EXTERNAL_SERVICE_URL ?? "")) {
    throw new TRPCError({
      code: "INTERNAL_SERVER_ERROR",
      message: "Heartbeat service API key is not configured (EXTERNAL_SERVICE_KEY).",
    });
  }
...
const callExternalService = async <T>(
  rpc: string,
  ...
  if (!response.ok) {
    ...
    throw mapExternalServiceError(response, detail, rpc);
  }
...
export async function createHeartbeatJob(
  ...
  return callExternalService<{ taskUid: string; nextExecutionAt?: string | null }>(
    "CreateHeartbeatJob",
    {
      name: job.name,
...
export async function updateHeartbeatJob(
  ...
  return callExternalService<{ nextExecutionAt?: string | null }>(
    "UpdateHeartbeatJob",
    body,
    userSession
  );
...
export async function deleteHeartbeatJob(
  ...
  await callExternalService("DeleteHeartbeatJob", { taskUid }, userSession);
}
